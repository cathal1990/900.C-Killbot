...
export class ZKillSubscriber {
                    ...
                    try {
                        ...
                        switch (subscription.subType) {
                            ...
                            case SubscriptionType.REGION:
                                systemData = await this.getSystemData(
                            ...
                            case SubscriptionType.ALLIANCE:
                                if (
                                    data.victim.alliance_id === subscription.id
                                ) {
                                    requireSend =
                                        subscription.killType ===
                                            KillType.LOSSES ||
                                        subscription.killType === undefined;
                                    color = "RED";
                                }
                                if (!requireSend) {
                            ...
                            case SubscriptionType.CHARACTER:
                                if (
                                    data.victim.character_id === subscription.id
                                ) {
                                    requireSend =
                                        subscription.killType ===
                                            KillType.LOSSES ||
                            ...
                            case SubscriptionType.GROUP:
                                if (data.victim.ship_type_id) {
                                    groupId = await this.getShipGroup(
    ...
    private async sendKill(
        ...
        await this.asyncLock.acquire("sendKill", async (done) => {
            ...
            const c = <TextChannel>(
                await this.doClient.channels.cache.get(channelId)
            );
            if (c) {
                let embedding = null;
                try {
                ...
                try {
                    ...
                    if (embedding?.error === false) {
                        const attackers = data.attackers.filter(
                            (a: any) => a.ship_type_id
                        );
                        const unknownShips =
                            data.attackers.length - attackers.length;
                        const attackerShips = [];

                        for (const att of attackers) {
                            try {
                                const shipName =
                                    await this.esiClient.getShipFromId(
                                        att.ship_type_id
                                    );
                                attackerShips.push(shipName);
                            } catch (error) {
                                console.error(
                                    `Error fetching ship name for ID ${att.ship_type_id}:`,
                                    error
                                );
                            }
                        }
                        const attackerShipNames = attackerShips.reduce(
                            (obj, name) => {
                        ...
                        const descriptionArray =
                            embedding?.result.ogDescription?.split(" ") || [];
                        const systemIndex = descriptionArray.indexOf("in");
                        const systemName =
                            descriptionArray[systemIndex + 1] || "NA";
                        ...
                        const message = `System: ${systemName}\n\nTime: ${formattedDate}\n\n\`\`\`Attackers: ${
                            data.attackers.length
                        }\n\n${shipCountsString}\n${
                            unknownShips > 0 ? "Unknown: " : ""
                        }${unknownShips > 0 ? String(unknownShips) : ""}\`\`\``;
                    ...
                    if (e instanceof DiscordAPIError && e.httpStatus === 403) {
                        try {
                            ...
                            await owner.send(
                                `The bot unsubscribed from channel ${c.name} on ${c.guild.name} because it was not able to write in it! Fix the permissions and subscribe again!`
                            );
                            console.log(
                                `Sent message to owner of ${c.guild.name} to notify him/her about the permission problem.`
                            );
                        } catch (e) {
                            console.log(e);
                        }
                        const subscriptionsInChannel = this.subscriptions
                            .get(guildId)
                            ?.channels.get(channelId);
                        if (subscriptionsInChannel) {
                            // Unsubscribe all events from channel
                            subscriptionsInChannel.subscriptions.forEach(
                                (subscription) => {
                                    this.unsubscribe(
                                        subscription.subType,
                                        guildId,
    ...
    public subscribe(
        ...
        if (!this.subscriptions.has(guildId)) {
            this.subscriptions.set(guildId, {
                channels: new Map<string, SubscriptionChannel>(),
            });
        }
        ...
        fs.writeFileSync(
            "./config/" + guildId + ".json",
            JSON.stringify(this.generateObject(guild)),
            "utf8"
        );
    }
    ...
    private generateObject(object: any): any {
        ...
        for (const key of keys) {
            if (object[key] instanceof Map) {
                newObject[key] = this.generateObject(
                    Object.fromEntries(object[key])
                );
        ...
        for (const file of files) {
            if (file.name.match(/\d+\.json$/)) {
                ...
                if (guildId && guildId.length > 0 && guildId[0]) {
                    ...
                    this.subscriptions.set(guildId[1], {
                        channels: this.createChannelMap(
                            parsedFileContent.channels
                        ),
                    });
                }
            }
    ...
    private createChannelMap(object: any): Map<string, SubscriptionChannel> {
        ...
        for (const key of keys) {
            map.set(key, {
                subscriptions: this.createSubscriptionMap(
                    object[key].subscriptions
                ),
            });
        }
        return map;
    ...
    private async getSystemData(systemId: number): Promise<SolarSystem> {
        return await this.asyncLock.acquire("fetchSystem", async (done) => {
            ...
            fs.writeFileSync(
                "./config/systems.json",
                JSON.stringify(Object.fromEntries(this.systems)),
                "utf8"
            );
    ...
    private async getShipGroup(shipId: number): Promise<number> {
        return await this.asyncLock.acquire("fetchShip", async (done) => {
            ...
            fs.writeFileSync(
                "./config/ships.json",
                JSON.stringify(Object.fromEntries(this.ships)),
                "utf8"
            );
    ...
    private async isInLimit(
        ...
        if (
            subscription.limitType === LimitType.SYSTEM &&
            limit.indexOf(systemData.id.toString()) !== -1
        ) {
            return true;
        }
        if (
            subscription.limitType === LimitType.CONSTELLATION &&
            limit.indexOf(systemData.constellationId.toString()) !== -1
        ) {
            return true;
        }
        return (
            subscription.limitType === LimitType.REGION &&
            limit.indexOf(systemData.regionId.toString()) !== -1
        );
    }
}
